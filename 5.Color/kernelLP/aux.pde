...
void histo(int[] hist, PImage img, String chn) {
  ...
  for(int k = 0; k < img.pixels.length; k++) {
    ...
    if (chn.equals(brightness)) {
      val = int(brightness(img.pixels[k]));
    }
    hist[val]++;
  }
}

void histo(int[] hist, double[][] im) {
  // Calculate the histogram
  int rows = im.length;
  int cols = im[0].length;
  int val = 0;
...
void brightnessEQ(double[][] im, double[] tf) {
  // equalization
  int rows = im.length;
  int cols = im[0].length;
  for(int row = 0; row < rows; row++) {
    for(int col = 0; col < cols; col++) {
      int bright = (int)(im[row][col]* 255.0);
...
void brightnessPosterisation(PImage out, double[][] in, int n, float[][] colormap) {
  ...
  for (int k = 0, row = 0, col = 0; k < out.pixels.length; k++, row= k% rows, col = int(k / rows)) {
    for(l = 0; l < n; l++) {
      if(in[row][col] > levels[l] && in[row][col] < levels[l + 1]) {
        //out.set(row, col, color((float) levels[l]*255, (float) levels[l]*255, (float) levels[l]*255));
