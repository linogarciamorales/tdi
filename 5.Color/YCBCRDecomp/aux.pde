...
void convert2double(double[][] im, PImage img, String chn) {
  ...
  for(int k = 0, row = 0, col = 0; k < img.pixels.length; k++, row= k% rows, col= int(k/ rows)) {
    ...
    double R = map(red(img.pixels[k]), 0, 255, 0, 1.0);
    double G = map(green(img.pixels[k]), 0, 255, 0, 1.0);
    double B = map(blue(img.pixels[k]), 0, 255, 0, 1.0);
    if(chn.equals(red)) {
      im[row][col] = R;
    }
    if(chn.equals(green)) {
    ...
    if(chn.equals(Y)) {
      ...
      im[row][col] = map((float) Y, 16, 235, 0, 1.0);
      //im[row][col] = (0.299 * R) + (0.5873 * G) + (0.114 * B);
    }
    if(chn.equals(Cb)) {
      double Cb = 128 + (-37.797 * R) + (-74.203 * G) + (112.0 * B);
      im[row][col] = map((float) Cb, 16, 240, 0, 1.0);
      //im[row][col] = (-0.1687 * R) + (-0.3313 * G) + (0.5 * B);
    }
    if(chn.equals(Cr)) {
      double Cr = 128 + (112.0 * R) + (-93.786 * G) + (-18.214 * B);
      im[row][col] = map((float) Cr, 16, 240, 0, 1.0);
      //im[row][col] = (0.5 * R) + (-0.4187 * G) + (-0.0813 * B);
    }
  }
}
