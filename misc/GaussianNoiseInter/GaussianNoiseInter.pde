...
void draw() {
  ...
  println(std_in);
  double SNR = mouseX*2;
  double std_out = std_in/ SNR;

  double[][] im_noised = new double[N][M];
  gaussianNoiseGen(im_noised, im_in, 0, std_out);
...
void gaussianNoiseGen(double[][] im_o, double[][] im_i, double mu, double std) {
  // Iterate over image
  for (int x = 0; x < im_o.length; x++){
    for (int y = 0; y < im_o[0].length; y++) {
      double noise = mu+ std* randomGaussian();
