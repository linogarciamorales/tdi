...
double findMax(double[] a) {
  double maxi = Double.MIN_VALUE; //lowest possible value of an double.
  int rows = a.length;
  for(int row=0; row < rows; row++) {
    if(a[row] > maxi) {
      maxi = a[row];
...
double findMin(double[] a) {
  double mini = Double.MAX_VALUE; //lowest possible value of an double.
  int rows = a.length;
  for(int row=0; row < rows; row++) {
    if(a[row] < mini) {
      mini = a[row];
...
void printarraybin(double[][] a, int row_start, int col_start, int rows, int cols) {
  for(int row = row_start; row < rows+row_start; row++) {
    double[] r = new double[cols];
    for(int col = col_start, k = 0; col < cols+col_start; col++, k++) {
        r[k] = a[row][col];
...
double[][] MAT(double[][] im) {
  ...
  for (int row = 0; row < rows; row++) {
    for (int col = 0; col < cols; col++) {
      mat[row][col] = im[row][col]* min(min(row, rows-row-1), min(col, cols-col-1));
    }
  }
  return mat;
}
...
double[][] MAT(double[][] im) {
  ...
  for (int row = 1; row < rows-1; row++) {
    for (int col = 1; col < cols-1; col++) {
      double[] values = { m[row-1][col-1]+d2, m[row-1][col]+d1, 
                          m[row-1][col+1]+d2, m[row][col-1]+d1,
                          m[row][col]};
      m[row][col] = findMin(values);
    }
  }
  ...
  for (int row = rows-2; row > 0; row--) {
    for (int col = cols-2; col > 0; col--) {
      double[] values = { m[row][col+1]+d1, 
                          m[row+1][col-1]+d2, m[row+1][col]+d1, 
                          m[row+1][col+1]+d2, 
                          m[row][col]};
      m[row][col] = findMin(values);
    }
...
double[][] MAX(double[][] im) {
  ...
  for (int row = 1; row < rows-1; row++) {
    for (int col = 1; col < cols-1; col++) {
      double[] values = { im[row-1][col], im[row+1][col], im[row][col-1], im[row][col+1] };
      double val = findMax(values);
      if (im[row][col] >= val) {
        m[row][col] = 1;
      }
    }
...
double[][] skel(double[][] im) {
  double[][] m = MAT(im);
  double[][] q = MAX(m);
  return q;
}

double[][] regionfilling(double[][] im, double[][] kernel, int[] pos, int K) {
  double[][] out = zeros(size(im));
  out[pos[0]][pos[1]] = 1;
  for(int k = 0; k < K-1; k++) {
    double[][] im_aux2 = zeros(size(im));
    double[][] im_aux1 = imcopy(out);
    dilation(im_aux2, im_aux1, kernel);
    logicalop(out, im_aux2, invert(im), and); 
  }
  logicalop(out, out, im, or); 
  return out;
}
