...
void hitandmiss(double im_out[][], double[][] im_in, double[][] m) {
  int rows = im_in.length;
  int cols = im_in[0].length;
  int M = m.length;
  double[][] im_e1 = new double[rows][cols];
  erosion(im_e1, im_in, m);
  rotateKernel(m, M+ 1);
  double[][] im_e2 = new double[rows][cols];
  erosion(im_e2, invert(im_in), m);
  logicalop(im_out, im_e1, im_e2, and);
}
