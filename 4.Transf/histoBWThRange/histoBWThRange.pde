...
void setup() {
  ...
  double[] alpha = {.25, .75};
  brightnessThresholdingRange(im_out, im_in, alpha);
...
void draw() {
  ...
  //save("histo_detalle_BW_th_range.png");
}
...
void brightnessThresholdingRange(double[][] out, double [][] in, double[] th) {
  // thresholding
  for(int k = 0, x = 0, y = 0; k < N* M; k++, x = k % N, y = int(k / N)) {
    if(in[x][y] > th[0] && in[x][y] < th[1]) {
      out[x][y] = 0.0;
    } else { 
      out[x][y] = 1.0;
    }
